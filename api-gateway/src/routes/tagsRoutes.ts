...
tagsRoute.all('/tags*', async (req: Request, res: Response) => {
	const requestId = uuidv4();
	const path = req.originalUrl.replace('/api/tags', '');
	...
	try {
		...
		await sendMessageToQueue(config.tagServiceQueue, message);
		...
		console.error('Ошибка маршрутизации для tags-service:', error);
