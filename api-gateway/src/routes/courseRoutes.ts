...
coursesRoute.all('/courses*', async (req: Request, res: Response) => {
	const requestId = uuidv4();
	const path = req.originalUrl.replace('/api/courses', '');
	...
	try {
		...
		await sendMessageToQueue(config.courseServiceQueue, message);
		...
		console.error('Ошибка маршрутизации для courses-service:', error);
