...
const tagsUrl = config.tagServiceUrl
const tagsQueue = config.queue;
...
export default async function connectRabbitMQ() {
    ...
    while (retries) {
        try {
            ...
            await channel.assertQueue(tagsQueue, { durable: false });

            console.log('[*] Ожидает сообщения. Для выхода нажать CTRL+C', tagsQueue);
            ...
            channel.consume(
                ...
                async (msg) => {
                    if (msg) {
                        ...
                        const url = `${tagsUrl}:${port}/${apiVer}/${path}`;
